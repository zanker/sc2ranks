...
module Armory
	class Command
		...
		def stop_daemon(index)
			process_name = "sc2armory_worker#{index}"
			...
			# Kill it
			begin
				Process.kill((RUBY_PLATFORM =~ /win32/ ? 'KILL' : 'TERM'), pid)
			...
			# Use our own method of killing processes if we're dealing with a specific stop
				...
				if @index.nil?
					...
					Dir["#{RAILS_ROOT}/tmp/pids/*.pid"].each do |path|
						...
						index = 1
						if worker.match(/sc2armory_worker([0-9]+)/)
							index = worker.match(/sc2armory_worker([0-9]+)/)[1].to_i	
						end
		...
		def run(worker_name = nil)
			...
			@files_to_reopen.each do |file|
				begin
					file.reopen File.join(RAILS_ROOT, 'log', 'sc2armory_worker.log'), 'a+'
					file.sync = true
					rescue ::Exception
				end
			end
